...
TcaplusDB RESTful API 基于通用 RESTful 协议进行设计，满足用户基于 RESTful 协议进行开发场景。文档基于`Generic表`作为介绍，`List表`暂不在此处进行介绍。详细的RESTful 接口使用手册请参考目录下：`document/TcaplusDB_RESTful_SDK_v2_User_Guide.pdf`。
